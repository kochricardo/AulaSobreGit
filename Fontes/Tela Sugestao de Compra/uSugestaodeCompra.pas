...
begin
  ...
  TFloatField(_cdsSugestaoCompra.FieldByName('UNITARIO')).DisplayLabel := 'Pço.Unit.';
  ...
  TFloatField(_cdsSugestaoCompra.FieldByName('PC_DESCONTO_PADRAO_COMPRA')).DisplayLabel := '% Desc.Padrão';
...
begin
  ...
  Printer.Canvas.TextOut(2800, nrLinha, 'Sugestão Compra p/'+nmCd);
...
begin
  try
     ...
     try
        ...
        excel.cells[02,01]:='Sugestão Compra p/'+nmCd;
        // Alterado conforme SS.: 11563 em 25/09/2015 (Vete)
        //excel.cells[03,01]:='Laboratório :' + FormatFloat('00000', nrFornecedor) + ' - ' + DaDosFornecedor.dsNomeFantasia;
        excel.cells[03,01]:='Laboratório :' + FormatFloat('00000', nrFornecedor) + ' - ' + DaDosFornecedor.dsGrupoFornecedor;
        excel.cells[04,01]:='Sugestão de Compra para  '+FormatFloat('##0',nrDias);
        ...
        excel.cells[07,01]:='Prazo Padrão : '+FormatFloat('##0',DaDosFornecedor.nrPrazoPedido);
        ...
        excel.cells[09,07]:=Consulta.Fields[04].DisplayLabel; // Preço Unit.   07
...
// --> cria o código html da página
  ...
  shtml:= shtml + '<title>'+'Sugestão de Compras'+'</title></head>' + #13#10;
