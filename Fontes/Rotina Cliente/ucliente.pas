...
begin
 ...
 try
    ...
    begin
     ...
     Prepared:=false;
     ExecProc;
     Result:='Erro Na Rotina Horario Caminhão -  '+FormatFloat('000000',ParamByName('ERRO_OU').AsInteger);
    if ParamByName('ERRO_OU').AsInteger=0 then
        result:=ParamByName('SAIDA_OU').AsString;
    end;
 except
    Result:='Erro Na Rotina Horario Caminhão -  '+FormatFloat('000000',_dmPrincipal._stpHorarioCaminhao.ParamByName('ERRO_OU').AsInteger);
end;
...
begin
    ...
    begin
     ...
     case StrToIntDef(idTipoBloqueio,0) of
          01 : begin
                  Result.stDescricaoBloqueio:='Alvará Vencido';
               end;
          02 : Result.stDescricaoBloqueio:='Alvará Vencido a mais de 90 dias';
          03 : begin
   ...
   //              MensagemRepresentante;
          ...
          08 : begin
                Result.stDescricaoBloqueio:='Data do Bloqueio - Credito e Cobrança';
 //               MensagemRepresentante;
                 Result.IdBloqueioPedido:='S';
               end;
     end;
...
begin
  ...
  try
    ...
    tempQuery.open;
    result.nmRepresentante:= 'Representante Não Cadastrado';
    result.nrTelefone     := '';
    if not tempQuery.IsEmpty then
    begin
