...
type
  ...
  //  qtSegurancaAcumulada: Double;
    ...
    idprecoAtual: Char;
    idBloqueiaPedido: Integer; // 0 Não Bleoqueado 1 Bloqueado ;
...
//    procedure MostraGraficoVendas(PnrProduto, PnrEmpresa: Integer);
    ...
    function CarregaUltimoPreçoEntrada(PnrFornecedor: Integer): Boolean;
  ...
  //          Mensagem('Curva Atualizada', MB_ICONINFORMATION)
    //      else
      //      Mensagem('Erro na Atualizacao da Curva', MB_ICONERROR);
          ...
          Mensagem('Erro na marcação', MB_ICONERROR);
      end;
      exit;
    end;
   if FormatDateTime('dd/mm/yyyy',dtBloqueiodeCompra)<>'01/01/2039' then
   begin
     if ConfirmaIcone('Produto Já esta Bloqueado para Compra Futura',
...
begin
  ...
  begin
   ...
   begin
        ...
        begin
          mensagem('verba Não disponivel para esse fornecedor !!!',MB_ICONWARNING);
  ...
  repeat
    ...
    begin
     ...
     idStatusCapa:=idStatus;
      ...
      if NrPedido > 0 then
      begin
        // Emiminando Itens Pedido não comprado.
        ...
        begin
          ...
          if _cdsProduto.State = dsEdit then
            _cdsProduto.FieldByName('ID_INATIVACAO_FUTURA').AsString := 'S';
             Mensagem('Marcação Efetuada', MB_ICONINFORMATION);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Mensagem('Desmarcação Efetuada', MB_ICONINFORMATION);
          exit;
        end
...
begin
  ...
  begin
   ...
   if DadosPedidoAbertuta.NrFornecedor <> NrFornecedor then
    begin
        Mensagem('Este pedido Não Corresponde ao Fornecedor do Pedido', 16);
        _lbeNrPedido.SetFocus;
        exit;
    end;
      ...
      Begin
        ...
        begin
            ...
            begin
              ...
              if nrVerba=0 then
              begin
                  mensagem('Verba Não Localizada!!!',MB_ICONWARNING);
...
//          _cdsPrePedido.FieldByName('ABTY_Y').AsFloat :=pcDescontoPadrao;
        ...
        begin
          ...
          begin
            Mensagem('Nr. Verba definida para este Pedido é ' +
      ...
      begin
        ...
        begin
          if not _cdsPrePedido.Locate('NM_MERCADORIA', _lbePesquisa.Text,
            ...
            _stbCompras.Panels[1].Text := 'Sem registro com essa descrição...'
          else
          begin
            nrProduto := _cdsPrePedido.FieldByName('CD_MERCADORIA').AsInteger;
            nrEmpresa := _cdsPrePedido.FieldByName('CD_EMPRESA_ORIGEM').AsInteger;
...
begin
   ...
   if FormatDateTime('dd/mm/yyyy',dtInativacaoFutura)<>'01/01/2039' then
   begin
     if ConfirmaIcone('Produto Já esta Marcado para Inativacao Futura',
...
begin
  ...
  begin
    ...
    begin
      ...
      Series3.Title := 'Est.Segurança';
      Series8.Title := 'Est.Máximo';
...
begin
    _stbCompras.Panels[1].Text := 'Dados Sugestão de Compra...';
   _stbCompras.Refresh;
...
begin
      if confirma('Esse pedido não gera pedido automatico, apenas sugere'+#13+
              ' e não altera as data de agenda do próximo pedido.'+#13+
              'Gerar pedido Exceção?',MB_DEFBUTTON2)=idYes then
        begin
         idAtualizaAgendaPedido:=false;
         TipoPedido:='EXCECAO';
         PedidoNovo(true);
...
//  if (_cdsProduto.FieldByName('ID_INATIVACAO_FUTURA').AsString = 'S') or (PrecoItem.vlPrecoLiquido=0) then
 ...
 if nrQtdadeSusgetaoPossivel>0 then
  ...
  begin
    ...
    stAlertaRestricao := 'Produto com Restrição de Compra.' + #13 +
      ' O mesmo poderá ser comprado.' + #13 +
      'Porém fale com Farmacêutico antes';

    _mmAlertaRecall.Lines.Add('Produto com Restrição de Compra.');
    _mmAlertaRecall.Lines.Add('O mesmo poderá ser comprado.');
    _mmAlertaRecall.Lines.Add('Porém fale com Farmacêutico antes.');
...
begin
  ...
  // Mostra Dado Preço lista , Desconto Padrao
...
begin
 ...
 if vlDesvioEMedia <= PvlMediaDiaria then
  ...
  // Somatorio qtSeguranca  (NÃO usado)
  ...
  // qtMaxima eh usado para Posição Grafico
...
begin
   case key of
     ...
     VK_F10 :  begin
                 if Confirma('Deseja Cancelar a Lista de Preço Fornecedor ?'+#13+
                             ' Atenção a Lista de Preço '+ _cdsListaPrecoAtiva.FieldByName('NR_LISTA') .AsString+ ' Sera Cancelada!!!'  ,MB_DEFBUTTON2)=IDYES then
                 begin
...
begin
    ...
    begin
       Mensagem('Pedido é CrossDocking não pode mudar o caminho..',MB_ICONWARNING);
       EXIT;
    end;
    ...
    begin
      if confirma('Deseja Mudar Operação CrossDocking ?',MB_DEFBUTTON2)=idYes then
      begin
        nrProduto := _cdsPrePedido.FieldByName('CD_MERCADORIA').AsInteger;
        nrEmpresa := _cdsPrePedido.FieldByName('CD_EMPRESA_ORIGEM').AsInteger;
       ...
       begin
         ...
         // Usado para força que pedido não sera crossDockin
         // idForcaPedidoLocal = false pedido crossdocking
         // idForcaPedidoLocal = true não sera crossdocking pedido crossdocking
       ...
       { idForcaPedidoLocal:=false;
        ...
        begin
          ...
          begin
            ...
            _cdsPrePedido.FieldByName('QUAY_Y').AsInteger := 0;
            _edtQtdadePedido.Text := FormatFloat('######0', qtdPedido);
            mensagem('Produto sem Preço',MB_ICONWARNING);
            exit;
          end;
...
procedure T_frmTelaCompras._dbgPrePedidoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if (Key in ['0' .. '9'] = true) then // tenho que colocar número por número?
  begin
...
//     _cdsProduto.filter:='id_venda_autorizada=''S''';
  ...
  _stbCompras.Panels[1].Text := 'Abrindo Lista Preço Fornecedor...';
  ...
  _stbCompras.Panels[1].Text := 'Abrindo Tabela Lista Preço Fornecedor...';
  _stbCompras.Refresh;
 ...
 if (_cdsVendaFornecedor<>nil) then
    if _cdsVendaFornecedor.IsEmpty then
        Mensagem('Não conseguiu abrir Tabela venda forncedor',MB_ICONWARNING);
...
//  and (cdoperadorLogisticoPedido = 1)
    ...
    _stbCompras.Panels[0].Text := 'Montando Previsão de Compra...';
  ...
  try
   try
    ...
    begin
       ...
       add('         CD_EMPRESA,');
       add('         ID_NFE_TRANSITO,');
       add('         PC_ICMS_COMPRA,'); //PC_ICMS_COMPRA //Léo
       add('         QT_MERCADORIA_TRANSITO,');
       add('         PC_IPI,');
       add('         CD_OPERACAO_REDE,');
       add('         VL_CUSTO_GERENCIAL,');
...
begin
  ...
  begin
   ...
   if _cdsCondicaoPagamento <> nil then
    ...
    _stbCompras.Panels[1].Text := 'Abrindo Tabela Ultimo Preço Ultima Entrada...';
    _stbCompras.Refresh;
     CarregaUltimoPreçoEntrada(NrFornecedor);
...
begin
  ...
  TIntegerField(_cdsTabelaLeadTime.FieldByName('QT_DIAS_MEDIO_LEADTIME')).DisplayLabel := 'LT. Médio';
  ...
  begin
    ...
    begin
      ...
      // idPedidoBloqueado := DadosPedidoAbertuta.idBloqueiodeDigitacao;
       ...
       if DadosPedidoAbertuta.NrFornecedor=0  then
      begin
        Mensagem('Pedido Não encontrado', 16);
        nrPedidoAlteracao:=0;
        _lbeNrPedido.SetFocus;
        exit;
      end;
      ...
      begin
        Mensagem('Este pedido Está Inativo', 16);
        _lbeNrPedido.SetFocus;
        exit;
      end;
      ...
      begin
        Mensagem('Este pedido De Operação de Rede Grandes Redes... '+#13+
                 'Não pode ser alterado.', 16);
        _lbeNrPedido.SetFocus;
        exit;
      ...
      begin
        Mensagem('Este pedido Está Bloqueado Para Alteração... ' + #13 +
          'Motivo mudança de Regra de Preço', 16);
        _lbeNrPedido.SetFocus;
        exit;
      end;
    end;
...
//    TIntegerField(TabelaPedido.FieldByName('QT_PENDENCIA')).DisplayWidth := 06;
    ...
    TFloatField(TabelaPedido.FieldByName('ABTY_Y')).DisplayLabel :=  'Desc.Padrão';
...
//  if (precoLista.PcIcmsCompra > 0) and
  ...
  begin
    mensagem('Item sem Preço ...',MB_ICONWARNING);
...
begin
  ...
  TFloatField(_cdsEstoqueEmpresa.FieldByName('VL_ESTOQUE_FORNECEDOR'))
    .DisplayLabel := 'Vlr.Preço Lista';
  TFloatField(_cdsEstoqueEmpresa.FieldByName('VL_ESTOQUE_FORNECEDOR'))
    .DisplayFormat := '###,###,##0.00';
  TFloatField(_cdsEstoqueEmpresa.FieldByName('VL_ESTOQUE_FORNECEDOR'))
    .DisplayWidth := 10;

  TFloatField(_cdsEstoqueEmpresa.FieldByName('VL_ESTOQUE_FINANCEIRO')).DisplayLabel := 'Vlr.Preço Custo';
  ...
  TFloatField(_cdsEstoqueEmpresa.FieldByName('VL_ESTOQUE_FABRICA')).DisplayLabel := 'Vlr.Pço Fabrica';
  ...
  TFloatField(_cdsEstoqueEmpresa.FieldByName('VL_ESTOQUE_VENDA')).DisplayLabel  := 'Vlr.Pço Venda';
...
begin
  ...
  TFloatField(_cdsEstoquePendencia.FieldByName('VL_PENDENCIA_FORNECEDOR')).DisplayLabel := 'Vlr.Preço Lista';
  ...
  TFloatField(_cdsEstoquePendencia.FieldByName('VL_PENDENCIA_CUSTO')).DisplayLabel := 'Vlr.Preço Custo';
  ...
  TFloatField(_cdsEstoquePendencia.FieldByName('VL_PENDENCIA_FABRICA')).DisplayLabel := 'Vlr.Pço Fabrica';
  ...
  TFloatField(_cdsEstoquePendencia.FieldByName('VL_PENDENCIA_VENDA')).DisplayLabel := 'Vlr.Preço Venda';
...
begin
  ...
  TFloatField(_cdsVendaMes.FieldByName('VT_FATURAMENTO_LISTA')).DisplayLabel :='Fat.Preço Lista';
  ...
  TFloatField(_cdsVendaMes.FieldByName('VT_FATURAMENTO_FABRICA')).DisplayLabel:= 'Fat.Pço.Fabrica';
...
begin
  ...
  TFloatField(_cdsExcessoEstoque.FieldByName('VL_EXCESSO_CUSTO')).DisplayLabel:= 'Vlr.Preço Custo';
  ...
  TFloatField(_cdsExcessoEstoque.FieldByName('VL_EXCESSO_LISTA')).DisplayLabel:= 'Vlr.Preço Lista';
  ...
  TFloatField(_cdsExcessoEstoque.FieldByName('VL_EXCESSO_VENDA')).DisplayLabel:= 'Vlr.Pço Venda.';
  ...
  TFloatField(_cdsExcessoEstoque.FieldByName('VL_EXCESSO_FABRICA')).DisplayLabel:= 'Vlr.Pço Fabrica.';
...
begin
   ...
   begin
    if PidMostraMensagem then
      Mensagem('Produto Marcado para Ser Inativado...'+#13+
               'Não pode ser comprado!!',MB_ICONWARNING);
     Result:=false;
     exit;
   end;
  ...
  if not PIdVendaAutorizada then
   begin
     if PidMostraMensagem then
      Mensagem('Produto Com a Venda Não Autorizada...'+#13+
               'Não pode ser comprado!!',MB_ICONWARNING);
...
//     getCondicaoPagamento(_cdsPedido.FieldByName('NR_CONDICAO_PAGTO_P').AsInteger);
     ...
     //    excel.Name:=dsNmPlanilha;
        ...
        if AnsiContainsStr(DadosValoresPedido.stObservacaoPedido,'Psicotrópicos') then
            if _chkBonificaPedido.Checked then
...
Function T_frmTelaCompras.CarregaUltimoPreçoEntrada(PnrFornecedor:Integer):Boolean;
var
  stQry: TStringList;
  stPedidoAtivos:String;
  ...
  begin
   ...
   begin
    if not IdTipoPedido then
         mensagem('Não ha Necessidade de Pedido Com Esse Paramentros',MB_ICONWARNING);
     stPedidos:='0';
   end;
...
begin
  ...
  SetCorPendencia(DadosPendencia.QtPedido,
   ...
   begin
      Mercadoria:=nil;
      mensagem('Item Não Localizado na Tabela de Produtos...',MB_ICONERROR);
      exit;
...
//                DadosPendencia.qtEstoqueCrossdocking+
  ...
  //Mostrando observação de motivo Inativação futura
  if FormatDateTime('dd/mm/yyyy',dtInativacaoFutura)<>'01/01/2039' then
  begin
  ...
  //Mostrando observação de motivo Bloqueio de Compra
...
begin
 ...
 begin
  ...
  begin
     Mensagem('Inclusão de Item não permitida'+#13+
              ' pedido CrossDocking e item não é  CrossDocking',MB_ICONWARNING);
     result:=false;
  end;
  if (not PidPedidoCrossdocking) and (PidItemCrossdocking='S') then
  begin
     Mensagem('Inclusão de Item não permitida'+#13+
              ' pedido não é CrossDocking e item é CrossDocking',MB_ICONWARNING);
     result:=false;
  end;
 end;
