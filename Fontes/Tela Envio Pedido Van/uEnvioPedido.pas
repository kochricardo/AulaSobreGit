...
begin
 ...
 END;
  ...
  begin
    Mensagem('Destino do Arquivo não está configurado No Sistema '+#13+
             ' Arquivo de Pedido não será gerado'+#13+
             ' Operação cancelada !!! '  ,16  );
     exit;
  end;
  ...
  begin
  try
     if confirma('Atenção'+#13+
                'A Confirmação deste Processo Transmitirá um Pedido'+#13+
...
begin
  ...
  TFloatField(_cdsPedidos.FieldByName('ABTP_P')).DisplayLabel:='% Desc.Padrão';
...
begin
  ...
  TStringField(_cdsItemPedido.FieldByName('DS_APRESENTACAO_MERCADORIA')).DisplayLabel:='Apresentação Produto';
  ...
  TFloatField(_cdsitemPedido.FieldByName('PUNY_Y')).DisplayLabel :='Preço Unit.';
  TFloatField(_cdsitemPedido.FieldByName('PUNY_Y')).DisplayFormat :='###,###,##0.00';

  TFloatField(_cdsitemPedido.FieldByName('PFBY_Y')).DisplayLabel := 'Preço Fornec.';
  ...
  TFloatField(_cdsitemPedido.FieldByName('VCCI_Y')).DisplayLabel :='Preço Custo';
  ...
  TFloatField(_cdsitemPedido.FieldByName('VL_MARGEM_PADRAO')).DisplayLabel := 'Vlr. Marg. Padrão';
  ...
  TFloatField(_cdsitemPedido.FieldByName('PC_BONIFICACAO')).DisplayLabel :='% Bonificação';
...
begin
    ...
    stParamentro:=stParamentro + insereCaracterEsquerda(cpunMediaPreço,' ',003);
...
BEGIN
  ...
  if EAN_FORNECEDOR='7891024000021' then
     if confirma('Fornecedor Tem Preços diferenciados para canal Farma e Alimentar !!'+#13+
                 'Este Pedido é para o canal Alimentar?',MB_DEFBUTTON2)=IDYES then
                 begin
...
begin
 ...
 // ACHA_PARAMENTRO_LABORATORIO(NR_LABORATORIO_LOCAL);
  ...
  BEGIN
    Mensagem('Destino do Arquivo não está configurado No Sistema '+#13+
             ' Arquivo de Pedido não será gerado'+#13+
             ' Operação cancelada !!! ',MB_ICONERROR );
  exit;
  END;
