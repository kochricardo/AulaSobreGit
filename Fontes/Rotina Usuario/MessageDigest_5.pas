...
function isHexEquals(const AValue1: AnsiString; const AValue2: AnsiString): boolean;
...
function hexToArrayByte(const AValue: AnsiString): TBytes;
var
  i,x : Integer;
begin
  i := 1;
  ...
  begin
    result[x] := StrToInt('$' + AValue[i] + AValue[i + 1]);
    inc(x);
    i := i + 2;
  end;
end;
...
begin
  ...
  begin
    a := hexToArrayByte(AValue1);
    b := hexToArrayByte(AValue2);
