...
begin
  ...
  begin
    ...
    begin
      { posicionar o combo na solução automática }
...
begin
  try
    ...
    begin
      { MERCADORIA NAO LOCALIZADA e critica não bloqueada }
...
begin
  ...
  begin
    try
      ...
      Application.MessageBox('NUMERO DE PEDIDO INVALIDO','ATENÇÃO',0 + 16);
      edNovoPedidoCompra.SetFocus;
      exit;
    end;

    if Application.MessageBox(' A CRITICA(NFe) DEVERÁ SER REPROCESSADA. ! ' + #13 +
                  ' DESEJA REALMENTE MUDAR O PEDIDO DE COMPRA ?','ATENÇÃO',4 + 32) = 6 then
    begin
      try
        ...
        Application.MessageBox('PEDIDO DE COMPRA ALTERADO COM SUCESSO','ATENÇÃO',0 + 64);
        mmObsComprador.Enabled:=true;
        mmObsComprador.Clear;
        mmObsComprador.SetFocus;
        ...
        Application.MessageBox('ERRO AO ALTERAR PEDIDO DE COMPRA','ATENÇÃO',0 + 16);
      end;
    end
    else
      Application.MessageBox('ALTERAÇÃO CANCELADA PELO USUARIO','ALTERAR PEDIDO DE COMPRA',0 + 64);
  end;
  ...
  begin
    try
      ...
      if nrComprador = 0 then
        raise Exception.Create('Comprador inválido.');
    except
      Application.MessageBox('COMPRADOR INVALIDO','ATENÇÃO',0 + 16);
      edNovoPedidoCompra.SetFocus;
      exit;
    end;

    if Application.MessageBox(' A CRITICA(NFe) DEVERÁ SER REPROCESSADA ! ' + #13 +
                              ' DESEJA REALMENTE MUDAR O COMPRADOR ?','ATENÇÃO',4 + 32) = 6 then
    begin
      try
        ...
        Application.MessageBox('COMPRADOR ALTERADO COM SUCESSO','ATENÇÃO',0 + 64);
        ...
        Application.MessageBox('ERRO AO ALTERAR COMPRADOR','ATENÇÃO',0 + 16);
      end;
    end
    else
      Application.MessageBox('ALTERAÇÃO CANCELADA PELO USUARIO','ALTERAR PEDIDO DE COMPRA',0 + 64);
...
begin
  ...
  begin
    if Application.MessageBox(' A CRITICA(NFe) DEVERÁ SER REPROCESSADA ! ' + #13 +
                  ' DESEJA REALMENTE REPROCESSAR A NFE ?','ATENÇÃO',4 + 32) = 6 then
    begin
      try
        // MUDA TIPO DA CRITICA ( ITENS ) PARA INATIVA
        AlteraDivergenciaItem_Inativar;
        ...
        cbSituacaoCritica.Enabled:=true;
        cbSituacaoCritica.Text:='006 - REPROCESSADA';
        Application.MessageBox('CRITICA LIBERADA PARA INATIVAÇÃO','ATENÇÃO',MB_ICONEXCLAMATION);
        mmObsComprador.Enabled:=true;
        mmObsComprador.Clear;
        mmObsComprador.Lines.Add('CRITICA REPROCESSADA');
        mmObsComprador.SetFocus;
      except
        Application.MessageBox('ERRO AO LIBERAR NFE PARA REPROCESSAMENTO','ATENÇÃO',MB_ICONERROR);
      end;
    end
    else
    begin
