...
begin
    ...
    begin
      pcDescontoAdicional:=vlParaVerba;
      if (vlParaVerba=0) or (vlParaVerba>100)  then
         pcDescontoAdicional:=100;
    end;
    ...
    begin
     ...
     Mercadoria        := GetMercadoriaNova(nrProduto,dgMercadoria,0,_cdsItemPedido.FieldByName('CD_EMPRESA').AsInteger);
     ...
     if (nrVerba=0) and (PcDescontoAdicional>0) and (pcVerba>=0) then
        pcVerba:=0;
     if (PcDescontoAdicional=0) and (pcVerba>=0) then
     begin
        pcVerba:=0;
        nrVerba:=0;
     end;
       ...
       begin
        ...
        begin
          if NOT AtualizaItemPedido(nrPedidoLocal,
                               ...
                               vlVerba,
                               _cdsItemPedido.fieldByName('VL_MEDIA_VENDA').AsFloat) THEN
