...
begin
  ...
  try
    ...
    begin
      mensagem('Não Atualizar numero do pedido',16);
      halt;
    end;
  end;
  if _dm._spNumeroPedido.Params[2].AsInteger > 0 then
  begin
    mensagem('Não Consegui Gerar numero do pedido',16);
    halt;
  end
  else
  begin
...
//     ParamByName('PpcDescontoFinanceiro').AsBCD := PpcDescontoFinanceiro;
   ...
   begin
     RESULT := False;
     //mensagem('Não Atualizou Item Pedido!!!',16);
     //exit;
   end
   else
     RESULT := True;
  ...
  try
    ...
    try
      ...
      begin
        ...
        if ExecSQL(False) <= 0 then
          mensagem('Não Cadastrou Parcelamento!!!',16);
      end;
    except
      on E: Exception do
      begin
...
begin
  ...
  try
    ...
    begin
      RESULT := False;
      mensagem('Produto não Cadastrado Para Este OPerador Logistico!!!' + #13 +
        ' Não pode fazer Pedido se não estiver Cadastrado!!!' + #13 +
        '***************************************************' + #13 +
        '***************************************************' + #13 +
        '***** Obs : Se Necessário Cadastrar, Falar com CPD ********' + #13 +
        '***************************************************' + #13 +
        '***************************************************',16);
    ...
    begin
      RESULT := False;
      mensagem('Erro Na Rotina que verifica se cliente Está Ativos !!!' + #13 +
        'Comunicar Desenvolvimento',64);
    end;
  end;
end;
(* ****************************************************** *)
{$ENDREGION 'Set Informação Produto'}
(* ****************************************************** *)
...
BEGIN
  ...
  EXCEPT
    BEGIN
      mensagem('Cod. Operador Inválido',16);
      QryTemp.Close;
      QryTemp := nil;
      QryTemp.Free;
    END;
...
begin
  ...
  Printer.Canvas.TextOut(0050, nrLinha, 'GAM - Genésio A. Mendes & Cia Ltda.'+'('+stUF+')');
  ...
  nrLinha := nrLinha - Printer.Canvas.Font.Height + 09;
  Printer.Canvas.TextOut(0050, nrLinha, 'Endereço Entrega....: '+stLogradouro+'  Bairro..: '+stBairro +'  Cidade..: ' +stCidade+'  CEP.....:'+stCep);
  nrLinha := nrLinha - Printer.Canvas.Font.Height + 09;
  Printer.Canvas.TextOut(0050, nrLinha, 'Observação..........: '+'Informar Nr. do Nosso Pedido '+inttostr(nrPedido)+' na Nota Fiscal/Danfe em Dados Adicionais');
  nrLinha := nrLinha - Printer.Canvas.Font.Height + 09;
...
begin
  ...
  Printer.Canvas.TextOut(0050, nrLinha, 'Código GAM');
  Printer.Canvas.TextOut(0700, nrLinha, 'Código Lab');
  Printer.Canvas.TextOut(1300, nrLinha, 'Qtdade');
  Printer.Canvas.TextOut(1800, nrLinha, 'Descrição do Produto');
  Printer.Canvas.TextOut(3700, nrLinha, '%. Desconto');
  Printer.Canvas.TextOut(4700, nrLinha, 'Preço Unit.');
...
begin
  Printer.Canvas.Font.name:='Courier New';
  printer.canvas.font.Size := 12;
  Printer.Canvas.TextOut(0050, nrLinha, 'GAM - Genésio A. Mendes & Cia Ltda.');
  printer.canvas.font.Size := 09;
  Printer.Canvas.Font.Style:=[];
//  nrLinha := nrLinha - Printer.Canvas.Font.Height + 09;
  Printer.Canvas.TextOut(4650, nrLinha, 'Data  :  ' + FormatDateTime('dd/mm/yyyy', dtSistema));
...
begin
  ...
  begin
    ...
    BeginDoc;
    nrLinha := nrLinha - Printer.Canvas.Font.Height + 10;
    nrLinha:=TopoPaginaPedido(nrPag, nrLinha, PnrPedido,DadosValoresPedido.dtPedido,'',DadosCD.stENDERECO+' Nº. '+DadosCD.nrLogradouro,
    ...
    begin
      if  GravaCapaPedido(nrPedidoNovo, // NROP_P,//  NUMBER(6)
                       ...
                       begin
                        ...
                        if DadosValoresPedido.nritensPedido>0 then
                        begin
                          stObs:='Ped.de Psicotróp.';
                          if PidBonificado then
                             stObs:='Ped.de Psicotróp.- Bonificado';
...
begin
  ...
  begin
     ...
     begin
       ...
       //Pedido Todo CrossDocking...com itens que não fazem crossDoking
       if (cdCrossDocking<>cdDestino) and (_cdstemp.recordcount>1) then
       begin
        nrPedidoNovo:=GeraNumeroPedido('C');
        if nrPedidoNovo>0 then
 ...
 try
   try
    ...
    if qryTemp.ExecSQL(false)<=0 then
      ...
      Mensagem('Erro ao Atualizar Inativação futura!!',16);
    end;
 finally
  FreeAndNil(qryTemp);
 end;
...
begin
  try
    ...
    begin
     ...
     Add('CASE');
     Add('WHEN nr_cd=1 THEN  ''CD Tubarão/SC''');
...
try
 try
   ...
   Mensagem('Erro na pesquisa de itens Já Faturado no pedido!!!',MB_ICONERROR);
  end;
finally
  FreeAndNil(qryTemp);
  FreeAndNil(stQry);
  ...
  try
   ...
   try
    ...
    begin
      ...
      add('case');
      Add('WHEN ESTOQUE.CD_EMPRESA=1 THEN ''CD. TUBARÃO''');
...
begin
  TRY
    ...
    begin
      ...
      ParamByname('PdtObservacao').AsDateTime := PdtObservação;
...
//     add('AND COMPRA_MERCADORIA.CD_EMPRESA=:PnrEmpresa');
   ...
   begin
     ...
     TStringField(_cdsTemp.FieldByName('ID_SITUACAO_AGENDA')).DisplayLabel:='Situação';
