object _frmMotivoInativacaoBloqueio: T_frmMotivoInativacaoBloqueio
  Left = 0
  Top = 0
  Caption = '_frmObservacaoInativacaoBloqueio'
  ClientHeight = 331
  ClientWidth = 819
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object StatusBar1: TStatusBar
    Left = 0
    Top = 312
    Width = 819
    Height = 19
    Panels = <
      item
        Width = 150
      end
      item
        Width = 50
      end>
  end
  object GroupBox1: TGroupBox
    Left = 0
    Top = 0
    Width = 819
    Height = 306
    Align = alTop
    Caption = 'Motivos'
    TabOrder = 1
    object Label1: TLabel
      Left = 16
      Top = 188
      Width = 35
      Height = 13
      Caption = 'motivo '
    end
    object Label2: TLabel
      Left = 18
      Top = 231
      Width = 58
      Height = 13
      Caption = 'Observa'#231#227'o'
    end
    object Label3: TLabel
      Left = 336
      Top = 188
      Width = 103
      Height = 13
      Caption = 'Bloquear Produto At'#233
    end
    object _labAvisoBloqueio: TLabel
      Left = 442
      Top = 209
      Width = 182
      Height = 13
      Caption = 'Sistema Bloquear'#225'  compra ate a data'
    end
    object _cbMotivo: TComboBox
      Left = 16
      Top = 204
      Width = 209
      Height = 21
      TabOrder = 0
    end
    object _btnGravaObservacao: TBitBtn
      Left = 16
      Top = 274
      Width = 153
      Height = 25
      Caption = 'Grava Observa'#231#227'o'
      DoubleBuffered = True
      Glyph.Data = {
        
        BFBFBFBFBF00}
      ParentDoubleBuffered = False
      TabOrder = 1
      OnClick = _btnGravaObservacaoClick
    end
    object _edtMotivoBloqueio: TEdit
      Left = 18
      Top = 246
      Width = 409
      Height = 21
      TabOrder = 2
    end
    object GroupBox2: TGroupBox
      Left = 2
      Top = 15
      Width = 815
      Height = 167
      Align = alTop
      TabOrder = 3
      object _DBGProdutosInativaBloqueia: TDBGrid
        Left = 2
        Top = 15
        Width = 811
        Height = 149
        Align = alTop
        Ctl3D = False
        DataSource = _dsProdutosInativaBloqueia
        ParentCtl3D = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDrawColumnCell = _DBGProdutosInativaBloqueiaDrawColumnCell
        OnDblClick = _DBGProdutosInativaBloqueiaDblClick
      end
    end
    object BitBtn1: TBitBtn
      Left = 648
      Top = 231
      Width = 129
      Height = 42
      Caption = 'Fechar'
      DoubleBuffered = True
      Glyph.Data = {
        
        89898989898989B1B1B1BFBFBFBFBFBF0000}
      ParentDoubleBuffered = False
      TabOrder = 4
      OnClick = BitBtn1Click
    end
    object _dtpBloquearAte: TDateTimePicker
      Left = 336
      Top = 204
      Width = 103
      Height = 21
      Date = 43405.000000000000000000
      Time = 43405.000000000000000000
      TabOrder = 5
    end
  end
  object _cdsProdutosInativaBloqueia: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = '_dspGenerica'
    RemoteServer = _dm._LocalConexao
    Left = 192
    Top = 64
  end
  object _dsProdutosInativaBloqueia: TDataSource
    DataSet = _cdsProdutosInativaBloqueia
    Left = 312
    Top = 64
  end
end
