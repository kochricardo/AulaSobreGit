...
type
  T_frmNfeVisualizaDanfe = class(TForm)
    ...
    qryconsulta_GEDCD_FORNECEDOR: TFMTBCDField;
    qryconsulta_GEDNOMP_P: TWideStringField;
    qryconsulta_GEDDT_EMISSAO: TSQLTimeStampField;
    qryconsulta_GEDDT_ATUALIZACAO: TSQLTimeStampField;
    qryconsulta_GEDNM_COMPRADOR_REDUZIDO: TWideStringField;
    qryconsulta_GEDDS_NFE_CHAVE: TWideStringField;
    qryconsulta_GEDNR_NFE: TFMTBCDField;
    qryconsulta_GEDCGCP_P: TFMTBCDField;
    qryconsulta_GEDNR_PEDIDO_COMPRA: TFMTBCDField;
    qryconsulta_GEDDS_NFE_XML: TBlobField;
    qryconsulta_GEDPC_REPASSE_ICMS: TFMTBCDField;
...
uses  URotinasGenericas, URelacaoNFeRecebida,uVisualizaCriticaPedidoNFe;
...
{    with dm.Planilha_padrao do
    begin
      ...
      WriteLabel(3,1,'DT EMISSÃO   : ' + EdDtEmissao.text);
 ...
 {     WriteLabel(6,1, 'CODIGO');
      ...
      TRY
        SaveToFile;
      EXCEPT
        MENSAGEM('Arquivo Não foi salvo!!!'+#13+
                 'Verifique se o mesmo Não está em uso');
      end;
    end;
    Screen.Cursor := 0;
...
procedure T_frmNfeVisualizaDanfe.ProcessaXML(nmArquivo:string);
  ...
  begin
    ...
    // TAG INFADIC - DADOS INFORMAÇÕES ADICIONAIS
...
procedure T_frmNfeVisualizaDanfe.CarregaDadosDANFE;
  ...
  procedure gravaArquivoLocal;
  var
    arquivoTemp : TextFile;
    nmArquivoAtual: string;
  begin
    nmArquivoAtual:=ExtractFilePath(Application.ExeName) + NM_ARQUIVO_XML;
    { arquivo nfe}
    AssignFile(arquivoTemp,nmArquivoAtual);
    if FileExists(nmArquivoAtual) then
      Append(arquivoTemp)
    else
      Rewrite(arquivoTemp);
    Write(arquivoTemp,AnsiToUtf8(qryconsulta_GED.FieldByName('DS_NFE_XML').AsAnsiString));
...
begin
  try
    ...
    if not qryconsulta_GED.isempty then
    begin
      NRO_PEDIDO_COMPRA:='0';
      CriaDataSet;
      //Mem_rec_XML.Clear;
      //Mem_rec_XML.Text:=AnsiToUtf8(qryconsulta_GEDDS_NFE_XML.AsString);
      ...
      // LÊ O XML E CRIA DATASET COM AS INFORMAÇÕES
      ProcessaXML(ExtractFilePath(Application.ExeName) + NM_ARQUIVO_XML);
      CDS_DADOS_ITEM.First;
      CDS_DADOS_DUPLICATA.First;

      // DADOS PRINCIPAIS DA NF
      EdDtEmissao.Text:=qryconsulta_GEDDT_EMISSAO.AsString;
      EdRazaoSocial.Text:=qryconsulta_GEDNOMP_P.AsString;
      ...
      NRO_PEDIDO_COMPRA:=qryconsulta_GEDNR_PEDIDO_COMPRA.AsString;
...
var
  sql : string;
begin
  sql:='SELECT CD_FORNECEDOR, NOMP_P, DT_EMISSAO, A.DT_ATUALIZACAO DT_ATUALIZACAO,NM_COMPRADOR_REDUZIDO,  '+
       ' DS_NFE_CHAVE,NR_NFE, CGCP_P, PC_REPASSE_ICMS,A.NR_PEDIDO_COMPRA,UTL_COMPRESS.LZ_UNCOMPRESS(A.DS_NFE_XML) DS_NFE_XML '+
       'FROM PRDDM.DC_NFE_COMPRA_GED A,PRDDM.DCPES,PRDDM.DC_FABRICANTE F,PRDDM.DC_COMPRADOR_GAM C '+
       'WHERE CGCP_P = NR_CNPJ_EMITENTE '+
       'AND F.CD_FABRICANTE = A.CD_FORNECEDOR '+
       'AND A.CD_COMPRADOR = C.NR_COMPRADOR '+
       'AND A.DS_NFE_CHAVE = '+ QuotedStr(dschave) +
       'ORDER BY DT_ATUALIZACAO,NOMP_P';

  qryconsulta_GED.Close;
  qryconsulta_GED.CommandText:='';
  qryconsulta_GED.CommandText:=sql;
  qryconsulta_GED.Open;
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        Application.MessageBox('CHAVE DE ACESSO NÃO LOCALIZADA','ATENÇÃO',MB_ICONWARNING);
    except
      on E:Exception do
...
begin
 ...
 { if formexiste(_frRelacaoNFeRecebida) = true then
    NRO_CHAVE_NFE:=_frRelacaoNFeRecebida.
  else
  begin
